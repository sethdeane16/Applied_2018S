...
int main(int argc, char *argv[])
  {
   ...
   if (1 != argc)
    ...
    /* Initialize Timing for data input with MACROS */
    #ifdef EN_TIME
    int data_reps = 1000;
    DECLARE_TIMER(data_input);
    START_TIMER(data_input);
    DECLARE_REPEAT_VAR(data_input_repeat_var);
    BEGIN_REPEAT_TIMING(data_reps, data_input_repeat_var);
    #endif
    ...
    /* Complete Timing for data input with MACROS */
    #ifdef EN_TIME
    END_REPEAT_TIMING;
    STOP_TIMER(data_input);
    PRINT_TIMER(data_input);
    PRINT_RTIMER(data_input, data_reps);
    #endif


    /* Initialize Timing for calculations input with MACROS */
    #ifdef EN_TIME
    int calc_reps = 2500000;
    DECLARE_TIMER(calculations);
    START_TIMER(calculations);
    DECLARE_REPEAT_VAR(calculations_repeat_var);
    BEGIN_REPEAT_TIMING(calc_reps, calculations_repeat_var);
    #endif
    ...
    #ifdef EN_TIME
    END_REPEAT_TIMING;
    STOP_TIMER(calculations);
    PRINT_TIMER(calculations);
    PRINT_RTIMER(calculations, calc_reps);
    #endif
